...
def load_image(filename, size=None):
    ...
    if size is not None:
        ...
        # PIL expects (width, height), while NumPy image shapes are (height, width).
        pil_size = (int(new_shape[1]), int(new_shape[0]))
        img = np.array(Image.fromarray(img).resize(pil_size, resample=Image.BILINEAR))
    return img
