...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat * x_centered * -0.5 * inv_std**3, axis=0)
    dmean = np.sum(-dx_hat * inv_std, axis=0)
    dmean += dvar * np.mean(-2.0 * x_centered, axis=0)
    dx = dx_hat * inv_std + dvar * 2.0 * x_centered / N + dmean / N
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dx = (inv_std / D) * (
        D * dx_hat - np.sum(dx_hat, axis=1, keepdims=True)
...
def conv_backward_naive(dout, cache):
    ...
    dx = dx_pad[:, :, pad:pad + H, pad:pad + W] if pad else dx_pad
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx, dw, db
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_flat, dgamma, dbeta = batchnorm_backward_alt(reshaped, cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    inv_std = 1.0 / np.sqrt(var + eps)
    x_hat_group = (x_group - mean) * inv_std
    x_hat = x_hat_group.reshape(N, C, H, W)
    out = gamma * x_hat + beta
    cache = (G, x_hat_group, inv_std, gamma, x_hat)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return out, cache
...
def spatial_groupnorm_backward(dout, cache):
    ...
    # This will be extremely similar to the layer norm implementation.        #
    ###########################################################################
    G, x_hat_group, inv_std, gamma, x_hat = cache
    N, C, H, W = dout.shape
    ...
    M = dx_hat.shape[2]
    dx_group = (inv_std / M) * (
        M * dx_hat - np.sum(dx_hat, axis=2, keepdims=True)
        - x_hat_group * np.sum(dx_hat * x_hat_group, axis=2, keepdims=True)
    )
