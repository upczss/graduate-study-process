...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.dot(z_i.reshape(-1), z_j.reshape(-1)) / (
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        for i, j in ((k, k + N), (k + N, k)):
            numerator = torch.exp(sim(out[i], out[j]) / tau)
            denominator = 0.0
            for m in range(2 * N):
                if m != i:
                    denominator = denominator + torch.exp(sim(out[i], out[m]) / tau)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = torch.sum(left * right, dim=1, keepdim=True)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    positive = torch.cat((sim_positive_pairs(out_left, out_right),
