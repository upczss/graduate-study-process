...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    image_batch = torch.stack([
        clip_preprocess(Image.fromarray(image)).to(device) for image in images
    ])
    ...
    pred_indices = torch.argmax(similarities, dim=1).cpu().tolist()
...
class CLIPImageRetriever:
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        top_indices = torch.topk(similarities, k=min(k, similarities.numel())).indices.cpu().tolist()
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
...
class DINOSegmentation:
    ...
    def train(self, X_train, Y_train, num_iters=500):
        ...
        ############################################################################
        self.model.train()
        X_train = X_train.reshape(-1, X_train.shape[-1]).to(self.device)
        Y_train = Y_train.reshape(-1).to(self.device)
    ...
    def inference(self, X_test):
        ...
        logits = self.model(X_test.reshape(-1, X_test.shape[-1]).to(self.device))
        pred_classes = torch.argmax(logits, dim=1).reshape(original_shape)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return pred_classes
